...
const double declinationAngle = 0.27925268;
...
Compass::Compass() : i2cAddr_(0) {}

int8_t Compass::begin()
{
    uint8_t buff[3];
...
int8_t Compass::setGain(uint8_t gain)
{
    uint8_t data = 0;
...
int8_t Compass::readRaw(int16_t * x, int16_t * y, int16_t * z)
{
    if (i2cAddr_ == 0)
...
int8_t Compass::readGauss(double * x, double * y, double * z)
{
    int16_t x_r, y_r, z_r;
...
int8_t Compass::readBearingDeg(double * heading)
{
    int16_t x, y, z;

    int8_t ret = readRaw(&x, &y, &z);

    double tan_heading = atan2(y, x) + declinationAngle;
...
void Compass::conv2Byte2Signed16(uint8_t LSB, uint8_t MSB, int16_t * dest)
{
    *dest = 0;
...
int8_t Compass::i2cReadBytes(uint8_t reg, uint8_t *data, uint8_t len)
{
    if (i2cAddr_ == 0)
        return HMC5833L_ERROR_NOT_INITIALIZED;
...
int8_t Compass::i2cWriteByte(uint8_t reg, uint8_t data)
{
    if (i2cAddr_ == 0)
        return HMC5833L_ERROR_NOT_INITIALIZED;
